...
# ----------------------------------------------------------------------------
# AI FUEL PREDICTION MODEL (RandomForestRegressor trained on simulated data)
# ----------------------------------------------------------------------------

@st.cache_resource(show_spinner=False)
def train_fuel_model():
    """
    Trains a RandomForestRegressor on simulated trip data that captures
    the physical relationship between distance, cargo load, truck capacity,
    and base fuel efficiency, plus realistic noise (traffic, terrain, driving
    style, AC load, idling at tolls, etc).
    """
    ...
    # Load can be anywhere from empty to slightly over-rated capacity
    loads = rng.uniform(0, 1.05, n_samples) * max_loads

    load_ratio = np.clip(loads / np.maximum(max_loads, 0.1), 0, 1.1)

    # Heavier relative load reduces effective mileage (non-linear penalty)
    ...
    # Realistic noise: traffic congestion, ghat/hill sections, idling at toll
    # plazas, AC usage, driver behavior
...
def fetch_osrm_route(origin_latlon, dest_latlon):
    """
    Calls the public OSRM demo server to fetch a real, road-following route
    between two coordinates. Returns geometry as a list of (lat, lon) tuples,
    total distance in km, and duration in minutes.
    """
...
# ----------------------------------------------------------------------------
# HIGHWAY AMENITY SIMULATION (placed exactly on the real route polyline)
# ----------------------------------------------------------------------------
...
def generate_route_amenities(route_coords, distance_km, truck_key, seed=7):
    """
    Places simulated real-world amenities at points sampled along the
    ACTUAL OSRM road polyline, so every marker sits on the highway itself.
    """
    ...
    # --- Dhabas / rest areas: every ~100-150 km ---
    ...
    # --- City no-entry / bypass alerts: near origin (first 8%) & destination (last 8%) ---
    for frac in (0.04, 0.96):
        idx = int(n_points * frac)
        idx = min(max(idx, 0), n_points - 1)
...
def build_route_map(route_coords, amenities, origin_name, origin_coord, dest_name, dest_coord):
    ...
    # Amenity clusters (grouped layers for toggling)
    layer_groups = {}
...
def assign_return_load(current_dest_city, truck_key, seed=None):
    rng = random.Random(seed)
    candidate_cities = [c for c in CITY_COORDS.keys() if c != current_dest_city]
    return_dest_city = rng.choice(candidate_cities)
    ...
    try:
        ...
        # Fallback to great-circle estimate scaled for road factor, only if
        # the live routing service is briefly unreachable.
        ...
        distance_km = round(straight_km * 1.25, 1)  # road-factor approximation

    specs = TRUCK_SPECS[truck_key]
    return_load_ton = round(rng.uniform(0.3, specs["max_load_ton"]), 1)

    payout = (distance_km * PAYOUT_PER_KM) + (return_load_ton * PAYOUT_PER_TON)
    payout = round(payout, -1)  # round to nearest 10 INR

    pickup_window = datetime.now() + timedelta(hours=rng.randint(1, 6))

    commodities = ["Textiles", "Electronics", "FMCG Goods", "Packaged Foods", "Construction Hardware", "Auto Parts", "Agro Produce"]
    ...
    if cargo_load > specs["max_load_ton"] * 1.1:
        ...
        with st.spinner("Fetching real highway route from OSRM and running AI fuel prediction..."):
            try:
                ...
                st.session_state.return_load = None  # reset any prior return-load assignment
            except requests.exceptions.RequestException:
                st.error(
                    "🚫 Could not reach the OSRM routing service. Please check your internet connection "
                    "and try again."
                )
                st.session_state.trip_computed = False
...
if st.session_state.trip_computed:
    ...
    st.caption(
        "Green line follows actual roads via OSRM. Toggle amenity layers using the control in the top-right of the map."
    )
