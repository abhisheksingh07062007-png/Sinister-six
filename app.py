...
# Custom High-Contrast Styling (Dark Background + Visible Text)
st.markdown("""
    <style>
    ...
    /* Telemetry Metric Cards - Dark Container with White/Neon High-Contrast Text */
...
# Static Random Parameters (Prevent map re-rendering loops)
if "traffic_delay_min" not in st.session_state or manual_scan:
    st.session_state.traffic_delay_min = random.randint(25, 55)
    st.session_state.traffic_density = random.choice(["HIGH CONGESTION", "MODERATE BLOCKAGE", "CRITICAL JAM"])

traffic_delay_min = st.session_state.traffic_delay_min
traffic_density = st.session_state.traffic_density

# 3. MAP DISPLAY
st.markdown("### 🗺️ REAL HIGHWAY ROUTE NAVIGATION & DYNAMIC TRAFFIC SCAN")
st.caption(f"🛰️ **Corridor Status:** {traffic_density} (+{traffic_delay_min} min delay)")
...
folium.PolyLine(direct_road_geometry, color="#f43f5e", weight=5, opacity=0.85, dash_array="6, 8").add_to(m)
folium.PolyLine(full_bypass_geometry, color="#10b981", weight=6, opacity=0.95).add_to(m)
...
folium.Marker([28.5500, 77.1200], popup="☠️ Critical Accident Zone", icon=folium.Icon(color="black", icon="skull", prefix="fa")).add_to(m)
folium.Marker([28.1800, 77.2500], popup="🏢 FASTag Toll Plaza", icon=folium.Icon(color="orange", icon="building", prefix="fa")).add_to(m)
folium.Marker([28.2800, 76.9500], popup="🏥 Trauma Hospital Center", icon=folium.Icon(color="red", icon="hospital", prefix="fa")).add_to(m)
folium.Marker([28.0500, 77.3800], popup="⛽ Diesel Fuel Station", icon=folium.Icon(color="blue", icon="gas-pump", prefix="fa")).add_to(m)
...
st.warning(
    ...
    f"• **☠️ ACCIDENT REPORTED:** Main Highway corridor flagged with **{traffic_density}** (+{traffic_delay_min} Min Delay).\n\n"
    f"• **AUTONOMOUS REROUTE:** Fleet automatically bypasses bottleneck via **KMP Highway Network** ➔ Saved **{traffic_delay_min} mins**."
)

# 4. TELEMETRY CARDS (CLEAR DARK CARDS WITH WHITE TEXT)
