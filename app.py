...
if st.session_state.trip_computed:
    ...
    st.subheader("📱 Driver Dashboard — Trip Completion & Return Load")

    st.write(
        f"**Driver:** {summary['driver_name']}  |  **Truck:** {summary['truck_type']}  |  "
        f"**Route:** {summary['from_city']} → {summary['to_city']}"
    )

    complete_trip = st.button("✅ Mark Trip Complete & Assign Return Load", use_container_width=True)

    if complete_trip:
        with st.spinner("Unloading cargo and matching best available return load..."):
            return_load = assign_return_lo
